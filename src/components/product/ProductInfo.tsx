...
export default function ProductInfo({ product }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {specs.length > 0 && (
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-wide text-gray-500 mb-3">Specs</h2>
      ...
      {product.includes && product.includes.length > 0 && (
        <div>
          <Link to={`/products/${product.slug}/includes`} className="text-sm font-medium text-blue-600 hover:underline">
            See what&apos;s included →
          </Link>
        </div>
      )}
